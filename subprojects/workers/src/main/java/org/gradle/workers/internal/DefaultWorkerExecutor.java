...
public class DefaultWorkerExecutor implements WorkerExecutor {
    ...
    public void submit(Class<? extends Runnable> actionClass, Action<? super WorkerConfiguration> configAction) {
        ...
        try {
            spec = new SerializingActionExecutionSpec(actionClass, description, configuration.getForkOptions().getWorkingDir(), configuration.getParams(), createActionInstantiator());
        } catch (Throwable t) {
    ...
    private Instantiator createActionInstantiator() {
        DefaultServiceRegistry actionServices = new DefaultServiceRegistry();
        actionServices.add(WorkerExecutor.class, this);
