...
public class WorkerServer implements WorkerProtocol<ActionExecutionSpec> {
    private static final Instantiator defaultInstantiator = new DefaultInstantiatorFactory(new AsmBackedClassGenerator()).inject();

    @Override
    public DefaultWorkResult execute(ActionExecutionSpec spec) {
        try {
            Class<? extends Runnable> implementationClass = spec.getImplementationClass();
            Instantiator instantiator = spec.getActionInstantiator() != null ? spec.getActionInstantiator() :defaultInstantiator;
